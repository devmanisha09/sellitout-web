...
export default class PsApiService extends PsApi {
    ...
    **/
    ...
    ///Follower Item List
    static async getFollowerItemListByUserId<T extends PsObject<T>>(obj: T, limit: Number, offset: Number, loginUserId: String): Promise<PsResource<T[]>> {
    ...
    ///
    static async getChatHistoryList<T extends PsObject<T>>(obj: T, postData: any): Promise<PsResource<T[]>> {
        const url: String = PsUrl.ps_chat_history_url;
        return await PsApi.postData<T, T[]>(obj, url, postData);
    }
    ...
    static imageUrl = (path: String, size: number = 0) => {
        if(size == 1) {
            return PsConfig.base + "/uploads/thumbnail/" + path;
        }else {
            return PsConfig.base + "/uploads/" + path;
        }
    }
