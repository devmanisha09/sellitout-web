<template>
    <div class="w-mobile sm:w-median lg:w-large mx-auto">
        ...
        <div class="">
            ...
            <div class="flex flex-wrap sm:flex-row justify-between  mb-4 mt-32">
                <div class="flex items-center ">
                    ...
                    <ps-route-link
                        :to="{name: 'item-list',
                        query: {
                        catId: productProvider.product?.data?.category?.catId,
                        catName: productProvider.product?.data?.category?.catName } }">
                        <ps-label
                            class="hover:underline cursor-pointer font-medium lg:text-sm sm:text-xs text-xxs">
                    ...
                    <ps-route-link :to="{
                        ...
                        query: {
                            catId: productProvider.product?.data?.category?.catId,
                            catName: productProvider.product?.data?.category?.catName,
                            subCatId: productProvider.product?.data?.subCategory?.id,
                            subCatName: productProvider.product?.data?.subCategory?.name }  }">
                        <ps-label
                            class="hover:underline cursor-pointer font-medium lg:text-sm sm:text-xs text-xxs">
                    ...
                    <div class="flex flex-col w-1/2">
                        ...
                        <div class=" mt-2">
                            <p class="flex flex-auto">
                                ...
                                <ps-label class="ml-2 mt-1 lg:text-base text-xs font-light"> {{ productProvider.product.data ? productProvider.product?.data?.itemPriceType?.name : '' }} </ps-label>
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-span-2 sm:col-span-3  lg:col-span-4 w-full">
                    ...
                    <div v-if="loginUserId != productProvider.product?.data?.user?.userId" class="flex space-x-3 text-sm font-medium mt-2">
                        ...
                        <ps-route-link class="flex flex-grow" :to="{
                            name: 'chat',
                            query: {
                                buyerUserId : loginUserId,
                                sellerUserId : productProvider.product?.data?.addedUserId,
                                itemName : productProvider.product?.data?.title,
                                itemId : productProvider.product?.data?.id,
                                itemImageName : productProvider.product?.data?.defaultPhoto.imgPath,
                                itemPrice : productProvider.product?.data?.price,
                    ...
                    <div v-if="isPromote" class=" flex flex-row ">
                        ...
                        <div v-if="loginUserId == productProvider.product?.data?.user?.userId" class="flex flex-row text-sm font-medium mt-2">
                            ...
                            <ps-route-link class="flex flex-grow lg:me-2 me-1" :to="{ name: 'item-entry' ,query: { itemId: productProvider.product?.data?.id }}">
                    ...
                    <div v-else class="">
                        ...
                        <div v-if="loginUserId == productProvider.product?.data?.user?.userId" class="cursor-pointer flex flex-row text-sm font-medium mt-2">
                            ...
                            <ps-route-link class="lg:me-2 me-1 flex flex-grow" :to="{ name: 'item-entry' ,query: { itemId: productProvider.product?.data?.id }}">
                        ...
                        <div class="p-4 bg-primary-000 dark:bg-primaryDark-black rounded-xl lg:rounded-2xl">
                            ...
                            <div class="flex items-start justify-between leading-none" >
                                <div class="flex items-start no-underline text-black cursor-pointer">
                                    <div class="flex">
                                        <img alt="admin" class="rounded-full bg-transparent w-10 h-10 flex items-center object-cover" :src="productProvider.imageUrl( productProvider.product?.data?.user?.userProfilePhoto )" @error="productProvider.defaultProfileImage" >
...
export default {
    ...
    setup() {
        ...
        function showGalleryDetail(gallery, galleryList ){
            ...
            if(productProvider.product.data.video.imgId != ''){
