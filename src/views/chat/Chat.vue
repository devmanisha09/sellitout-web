    ...
    <ps-content-container class=" h-screen ">
        <template #content>
            <div class="relative flex flex-col " style="height:90vh" > 
                ...
                <div class='mt-32 h-40 flex flex-col ' >
                    <div class='flex flex-row justify-between mb-2'>
                        ...
                        <ps-route-link :to="{
                            ...
                            <div class="flex flex-row content-end items-center cursor-pointer">
                                <img alt="Placeholder"  width="300px" height="200px" class="w-10 h-10 p-2 object-cover bg-white rounded-full me-2" :src="userProvider.imageUrl(userProvider.user?.data?.userProfilePhoto) " @error="chatHistoryProvider.defaultProfileImage">
                                <ps-label class=""> {{userProvider.user?.data?.userName }} </ps-label>
                            </div>
                        </ps-route-link>
                    </div>
                    <div v-if="dataReady" class='flex flex-row justify-between'>
                        <ps-route-link :to="{ name: 'item', query: { itemId: itemId, itemName: itemName  }}">
                            <div class='flex flex-row cursor-pointer'>
                                <img alt="Placeholder"  width="300px" height="200px" class="w-20 h-20 p-2 object-cover bg-white rounded-md" :src="chatHistoryProvider.imageUrl(itemImageName) " @error="chatHistoryProvider.defaultCarImage">
                    ...
                    <div v-for="chat in chats.data" :key="chat.id ?? ''">
                        <div class=" " v-if='chat.message != "" && chat.message != null '>
                            ...
                            <div v-else-if="chat.type == PsConst.CHAT_TYPE_IMAGE">
                                ...
                                <!-- Send by me -->
                                <div v-if='chat.sendByUserId == loginUserId' class='w-full flex justify-end ps-1/3 flex-row items-end h-128 mt-4'>
                                    <ps-label-caption-2 class="me-2 mb-2"> {{chat.timeString}} </ps-label-caption-2>
                                    <img @click="imageView(chat.message)"  width="300px" height="200px" alt="Placeholder" class="cursor-pointer w-auto h-128 p-2 object-cover  rounded-md" :src="chatHistoryProvider.imageUrl(chat.message) " @error="chatHistoryProvider.defaultCarImage">
                                </div> 

                                <!-- Receive -->
                                <div v-else class="pe-1/3 flex flex-row items-end h-128 mt-4">
                                    <img @click="imageView(chat.message)"  width="300px" height="200px" alt="Placeholder" class="cursor-pointer w-auto h-128 p-2 object-cover rounded-md" :src="chatHistoryProvider.imageUrl(chat.message) " @error="chatHistoryProvider.defaultCarImage">
                                    <ps-label-caption-2 class="ms-2 mb-2"> {{chat.timeString}} </ps-label-caption-2>
                                </div>                                 
                            </div>
...
export default {
    ...
    setup() { 
        ...
        // Get Data
        const buyerUserId = route.query.buyerUserId?.toString() ?? '';
        const sellerUserId = route.query.sellerUserId?.toString() ?? '';
        const itemName = route.query.itemName?.toString() ?? '';
        const itemId = route.query.itemId?.toString() ?? '';
        const itemImageName = route.query.itemImageName?.toString() ?? '';
        let itemPrice = route.query.itemPrice?.toString() ?? '';
        ...
        const chatFlag = route.query.chatFlag?.toString() ?? ''; 
        ...
        async function sendMessage(
            ...
            if(chatFlag == PsConst.CHAT_FROM_SELLER) {
                chatHistoryProvider.postChatHistory(itemId,  buyerUserId, sellerUserId, PsConst.CHAT_FROM_BUYER, '1' ,chatHolder.message);                    
            }else {
                chatHistoryProvider.postChatHistory(itemId,  buyerUserId, sellerUserId, PsConst.CHAT_FROM_SELLER, '1', chatHolder.message );
            }
